...
from transformers import Blip2Processor, Blip2ForConditionalGeneration, BertModel
...
class QueryTransformer(nn.Module):
    ...
    def forward(self, vision_features):
        return self.transformer(vision_features)
...
class LanguageModel:
    ...
    def forward(self, text_inputs, inputs_embeds=None):
        return self.model(**text_inputs, inputs_embeds=inputs_embeds)
...
class BLIP2Model(nn.Module):
    ...
    def forward(self, images, text_inputs):
        ...
        query_features = self.query_transformer(vision_features)
        outputs = self.language_model(text_inputs, inputs_embeds=query_features)
        return outputs
