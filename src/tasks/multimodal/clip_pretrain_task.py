...
# from src.models.wrappers.vlm.clip_wrapper import CLIPWrapper
# from src.tasks.losses.vlm_losses import SymmetricCrossEntropyLoss
...
#     """
#     def __init__(self, 
#                  model_config: CLIPWrapper,  # <--- **THE FIX (Part 1)**: This is now an object, not a config
#                  loss: SymmetricCrossEntropyLoss, # <--- **THE FIX (Part 1)**
#                  optimizer: DictConfig, 
#                  scheduler: DictConfig,
#                  **kwargs):
#         super().__init__()
#         # This line saves optimizer and scheduler configs, but not the giant model
#         self.save_hyperparameters("optimizer", "scheduler") 
...
#             # Check if the predicted index points to a correct item
#             acc = targets[
#                 torch.arange(batch_size, device=self.device), pred_idx_per_image
#             ].mean()
...
#     def configure_optimizers(self):
#         # We use self.hparams since we saved them
#         optimizer = hydra.utils.instantiate(
#             self.hparams.optimizer, params=self.parameters()
#         )
